...
public class CopyOfSimpleTelnetConnection4 implements CommandLine {
...
//	private JSimpleTerminal debugTerminal;
	...
	public CopyOfSimpleTelnetConnection4(SimpleNetworkDevice device, int sourcePort, Inet4Address destinationAddress, int destinationPort) throws IOException {
		this.device = device;
...
//				debugInputStreamWrite.flush();
			...
			try {
				while (true) {
					...
					synchronized (sendMonitor) {
						sendMonitor.wait(500);
					}
					
					if (sequence != lastSequence) break;
				}
			} catch (InterruptedException e) {}
		}
	}
