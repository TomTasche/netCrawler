...
public abstract class D_SSHClient implements CommandLine {
	...
	public D_SSHClient(String host, String username, String password)
			throws IOException {
		connect(host, username, password);
	}
	
	public D_SSHClient(InetAddress address, String username, String password)
			throws IOException {
		connect(address, username, password);
	}
	
	public D_SSHClient(InetSocketAddress socketAddress, String username,
			String password) throws IOException {
		connect(socketAddress, username, password);
	}
	
	public D_SSHClient(String host, int port, String username, String password)
			throws IOException {
		connect(host, port, username, password);
	}
	
	public D_SSHClient(InetAddress address, int port, String username,
			String password) throws IOException {
		connect(address, port, username, password);
	}
	
	public void connect(String login, String password) throws IOException {
		connect(login.split("@")[1], DEFAULT_PORT, login.split("@")[0],
				password);
	}
	...
	public void connect(InetSocketAddress socketAddress, String username,
			String password) throws IOException {
		connect(socketAddress.getAddress(), socketAddress.getPort(), username,
				password);
	}
